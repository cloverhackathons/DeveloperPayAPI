    ...
    public class WebPay
    {
        ...
        public static void Main(string[] args)
        {
            ...
            if ((int)response.StatusCode >= 200 && (int)response.StatusCode <= 299)
            {
                ...
                byte[] modulusBytes = new BigInteger(modulus, 10).getBytes();
                byte[] exponentBytes = new BigInteger(exponent, 10).getBytes();
                ...
                byte[] encryptedCard = rsa.Encrypt(Encoding.UTF8.GetBytes(prefix + cardNumber), true);
